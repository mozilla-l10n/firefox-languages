...
def main():
    ...
    for locale in shipping_locales:
        ...
        transform_file = os.path.join(
            node_folder, "cldr-misc-full", "main", cldr_locale, "contextTransforms.json"
        )
        ...
        if os.path.isfile(transform_file):
            with open(transform_file) as data_file:
                try:
                    ...
                    languages[locale]["transform-type"] = json_data["main"][
                        cldr_locale
        ...
        if os.path.isfile(language_file):
            with open(language_file) as data_file:
                try:
                    json_data = json.load(data_file)
                    languages[locale]["cldr-name"] = json_data["main"][cldr_locale][
                        "localeDisplayNames"
        ...
        if (
            languages[locale]["transform-type"] in ["titlecase-firstword", "---"]
            and language_name != "---"
        ):
    ...
    with open(file_name, "w") as output_file:
        json.dump(languages, output_file, ensure_ascii=False, indent=2, sort_keys=True)
